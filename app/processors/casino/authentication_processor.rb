  ...
  def validate_login_credentials(username, password, context = nil)
    ...
    authenticators.each do |authenticator_name, authenticator|
      begin
        ...
        CASino.logger.error "Authenticator '#{authenticator_name}' (#{authenticator.class}) raised an error: #{e}"
      end

      next unless data

      authentication_result = { authenticator: authenticator_name, user_data: data }
      CASino.logger.info("Credentials for username '#{data[:username]}' successfully validated using authenticator '#{authenticator_name}' (#{authenticator.class})")
      break
    end
